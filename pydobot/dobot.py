...
class Dobot:
    ...
    # ------------------------------ conveyor (testing) ---------------------------------------------------------------
    def set_stepper_cmd(self, stepper_idx, stepper_enabled=True, stepper_speed=9.0, speed_scale=100):
        ''' stepper speed: scale_factor*(stepper_speed), scale_factor=100 '''
        emotor_msg = Message()
        emotor_msg.id = CommunicationProtocolIDs.SET_EMOTOR_CMD
        emotor_msg.ctrl = ControlValues.ONE # execute immediately
        emotor_msg.params = bytearray([])
        stepper_speed = (speed_scale*stepper_speed) if speed_scale != 0 else stepper_speed
        emotor_msg.params.extend(bytearray(struct.pack('<BBi', stepper_idx, stepper_enabled, stepper_speed)))
        return self._send_command(emotor_msg, wait=False)